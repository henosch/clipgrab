...
ClipGrab::ClipGrab()
{
    ...
    sslConfig.addCaCertificates(QSslCertificate::fromPath(":/crt/mozilla-root-cas.txt"));
...
void ClipGrab::getUpdateInfo()
{
    ...
    QUrl updateInfoRequestUrl("https://raw.githubusercontent.com/henosch/clipgrab/master/update-" + sys + ".xml");
...
void ClipGrab::startYoutubeDlDownload() {
    ...
    if (youtubeDlUrl.contains("/downloads/latest/youtube-dl", Qt::CaseInsensitive) ||
        youtubeDlUrl.contains("yt-dl.org", Qt::CaseInsensitive) ||
        youtubeDlUrl.isEmpty())
    {
        youtubeDlUrl = "https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp";
        settings.setValue("youtubeDlUrl", youtubeDlUrl);
    }
    ...
    connect(reply, &QNetworkReply::sslErrors, [=](QList<QSslError> errors) {
        for (int i = 0; i < errors.length(); i++) {
            ...
            QString certInfo = errors.at(i).certificate().issuerDisplayName() + " " + errors.at(i).certificate().subjectDisplayName() + " " +  errors.at(i).certificate().serialNumber() + " " + errors.at(i).errorString();
...
#if QT_VERSION >= 0x060000
            explorerProcess->startCommand("explorer.exe /select,"  + QDir::toNativeSeparators(video->getFinalFilename()));
#else
...
void ClipGrab::search(QString keywords) {
    ...
    if (keywords.isEmpty()) {
        ...
        if (langCode == "en" || langCode.isEmpty()) {
            ...
            QString query = "Popular on YouTube";
            if (langCode == "de") {
                query = "Beliebt auf YouTube";
            } else if (langCode == "fr") {
                query = "Populaire sur YouTube";
            } else if (langCode == "es") {
                query = "Popular en YouTube";
            } else if (langCode == "it") {
                query = "Popolare su YouTube";
            } else if (langCode == "ru") {
                query = "Популярное на YouTube";
            } else if (langCode == "pl") {
                query = "Popularne w YouTube";
            } else if (langCode == "pt") {
                query = "Popular no YouTube";
            } else if (langCode == "tr") {
                query = "YouTube'da Popüler";
            } else if (langCode == "nl") {
                query = "Populair op YouTube";
            } else if (langCode == "ja") {
                query = "YouTube で人気";
            } else if (langCode == "ko") {
                query = "YouTube 인기 동영상";
            } else if (langCode == "zh") {
                query = "YouTube 热门";
            } else if (langCode == "id") {
                query = "Populer di YouTube";
            } else if (langCode == "ca") {
                query = "Popular a YouTube";
            } else if (langCode == "cs") {
                query = "Populární na YouTube";
            } else if (langCode == "hu") {
                query = "Népszerű a YouTube-on";
            } else if (langCode == "ro") {
                query = "Populare pe YouTube";
            } else if (langCode == "hr") {
                query = "Popularno na YouTubeu";
            }

            currentSearch->fetchPlaylistInfo("ytsearch16:\"" + query + "\"");
        }
    } else {
